...
class Search {
    ...
    getRegrets (dNode, sample, payoffs) {
        let regrets = []
    ...
    sampleActions (strategies, gamma) {
        return strategies.map(function (strategy){
            let seed = Math.random() + gamma * strategies.length
            for (const i in strategy){
                seed -= strategy[i] + gamma
                if (seed <= 0) return i
            }
    ...
    run(dNode, state, gamma=0, info={depth:0, terminal:false}){
        ...
        const payoffs = this.run(dNode_, state_, gamma, info)

        const regrets = this.getRegrets(dNode, sample, payoffs)

        this.updateDecision(dNode, regrets, strategies)

        this.updateChance(cNode, payoff)
    ...
    exploitability (dNode) {
        ...
        for (const tuple of cartesian(dNode.actions)){
            ...
            const payoffs = this.expectedScore(dNode.chances.get(arrayToInt(tuple)))
...
for (let i = 0; i < n; ++i){
    ...
    let state = new State.TwoPlayerPureTree()
    state.expand(payoff=[0,0], depth=2, actions=[[0,1],[0,1],[0,1,2]])
    ...
    console.log(state.equilibrium)
    //console.log(search.exploitability(dNode))
    console.log(state.matrix.map(arr => arr.map(arrr => arrr.payoffs[0])))
    //dNode.chances.forEach((key, cNode) => console.log(search.expectedScore(cNode)))
    //state.check()
}
